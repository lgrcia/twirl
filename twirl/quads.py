...
def _reorder(q):
    ...
    q0 = q[q0i]; q123 = np.delete(q.copy(), q0i, axis=0)
...
def good_quads(quads, circletol=0.01):
    ...
    center = q0 + (q1-q0)/2
    in_circle = np.linalg.norm(quads - center[:, None, :], axis=2) <= r[:, None]*(1+circletol)
...
def _quad_hash(quad):
    ...
    return np.array([
        proj(c, a, u1, norm=True)/h, proj(d, a, u1, norm=True)/h,
        proj(c, a, u2, norm=True)/h, proj(d, a, u2, norm=True)/h
    ]), np.array([a, b])
